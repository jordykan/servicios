<template>
  <div class="page-header clear-filter" >
    ...
    ></div>
                        ...
                        <v-container >
                          <br>
                           <h3>Registro de Hojas de Servicio</h3>
            ...
            >
              <template v-slot:item.estado="{ item }">
                ...
                <v-toolbar flat color="indigo lighten-4">
                    <v-toolbar-title>Hoja de Servicio</v-toolbar-title>
                    <v-divider
                    class="mx-4"
                    inset
                    vertical
                    ...
                    <template v-slot:activator="{ on }">
                        <v-btn color="success" small class="mb-1" v-on="on"> Hoja de Servicio</v-btn>
                    </template>
                    <v-card>
                        <v-card-title>
                        ...
                        <v-container>
                            <v-row>
                              <v-col cols="6" sm="6" md="6">
                               <v-datetime-picker label="Horario de inicio de Servicio" locale="es" v-model="inicioServicio"> </v-datetime-picker>
                            </v-col>
                             <v-col cols="6" sm="6" md="6">
                               <v-datetime-picker label="Horario de Finalización de Servicio" locale="es" v-model="finalServicio"> </v-datetime-picker>
                            </v-col>
                            <v-col cols="12" sm="12" md="12">
                                <v-autocomplete  v-model="servicio" :items="servicios"  label="Servicio"></v-autocomplete>
                            </v-col>
                            ...
                            <v-col cols="4" sm="4" md="4" v-if="tipoServicio=='GRUA'">                        
                                  <v-select
                                   v-model="servicioLog"  
                                    :items="grua"                                
                                    label="Servicios de Grua"
                                  ></v-select>                       
                            </v-col>

                               <v-col cols="4" sm="4" md="4" v-if="tipoServicio=='HIAB'">                        
                                  <v-select
                                   v-model="servicioLog"  
                                    :items="hiab"                                
                                    label="Servicios de Hiab"
                                  ></v-select>                       
                            </v-col>


                             <v-col cols="8" sm="8" md="8" v-if="tipoServicio=='OTRO'" >                        
                                 <v-text-field v-model="servicioLog" label="Otro tipo de servicio"></v-text-field>                     
                            </v-col>
                               <v-col cols="12" sm="12" md="12" v-if="tipoServicio=='OTRO'" >  
                                 <v-textarea
                                 v-model="tipoServicioDescripcion"
                                  name="input-7-1"
                                  label="Describir el tipo de servicio"
                                ></v-textarea>                      
                            </v-col>
                            <v-col cols="12" sm="12" md="12">  

                                 <v-textarea
                                 v-model="servicioMaterialDescripcion"
                                  name="input-7-1"
...
export default {
        ...
        return{
           search:'',
            ...
            headers: [
                ...
                { text: 'Tipo', value: 'tipoServicio' },
                { text: 'Servicio', value: 'servicioLog' },
                { text: 'Descripcion', value: 'tipoServicioDescripcion' },
                { text: 'Detalles', value: 'servicioMaterialDescripcion' },
            ...
            servicioMaterialDescripcion:'',
            tipoMaterial:'',
            detalles:'',
            diesel:'',
            serv:'',
            aceite_l:'',
            servicios_log:['HIAB','GRUA','OTRO'],
            hiab:['6 TON','8 TON','10 TON','12 TON'],
            grua:['40 TON','50 TON','60 TON','80 TON','110 TON','120 TON','150 TON'],
            aceite_l2:'',
            onhire:'',
            consumoAmls:'',
            consumoCliente:'',
  ...
  components: {
   ...
   computed:{
     formTitle () {
      return this.editedIndex === -1 ? 'Nueva Hoja de Servicio' : 'Editar Hoja de Servicio'
    },
    },
  ...
  watch: {
   ...
   methods:{
         ...
         handleFileUpload(e){
             this.serviciodelog = this.tipoServicio 
             console.log(this.serviciodelog)
            },
        ...
        selectServicios(){
            ...
            axios.get('confirmarServicio/listServicioTerr',configuracion).then((response)=>{
      ...
      listar(){
          let me=this
           let header={"Token":this.$store.state.token};
           let configuracion= {headers: header}
          axios.get('hojaServicio/list',configuracion).then(function (response){
      ...
      guardar(){
          ...
          if(this.editedIndex >-1){
              ...
              axios.post('hojaServicio/add',{
                'servicio':this.servicio,
                'inicioServicio':this.inicioServicio,
                'finalServicio':this.finalServicio,
                'tipoServicio':this.tipoServicio,
                'usuario':this.$store.state.usuario._id,
                'servicioLog':this.servicioLog,
                'tipoServicioDescripcion':this.tipoServicioDescripcion,
                'servicioMaterialDescripcion':this.servicioMaterialDescripcion,
